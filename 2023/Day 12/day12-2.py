...
def calculate_arrangements(data):
    ...
    for record, checksum in data:
        ...
        for i, contiguous_length in enumerate(checksum):
            new_valid_positions = {}
            ...
            for position, count in valid_positions.items():
                ...
                for new_pos in new_positions_within_bounds:
                    is_contiguous_valid = (
                        new_pos + contiguous_length - 1 < len(record) and '.' not in record[new_pos:new_pos + contiguous_length]
                    )

                    is_last_checksum = i == len(checksum) - 1 and '#' not in record[new_pos + contiguous_length:]

                    is_not_last_checksum = i < len(checksum) - 1 and new_pos + contiguous_length < len(record) and record[new_pos + contiguous_length] != '#'

                    if is_contiguous_valid and (is_last_checksum or is_not_last_checksum):
                        new_valid_positions[new_pos + contiguous_length + 1] = new_valid_positions.get(new_pos + contiguous_length + 1, 0) + count
